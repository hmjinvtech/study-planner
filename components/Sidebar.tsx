        ...
        <nav className="flex-1 px-4 py-8 space-y-2 overflow-y-auto">
          {sidebarItems.map((item) => (
            <Link key={item.id} href={item.href}>
              <div
                className={`group flex items-center gap-4 px-4 py-3 rounded-lg transition-smooth cursor-pointer ${
                  isActive(item.href)
          ...
          <div className="flex items-center gap-3 hover:bg-white/10 p-3 rounded-lg transition-smooth cursor-pointer">
            ...
            {isOpen && (
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400 truncate">student@mail.com</p>
              </div>
            )}
          </div>
        </div>
